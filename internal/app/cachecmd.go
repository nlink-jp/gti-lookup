...
func runCache(args []string, stdout, stderr io.Writer) int {
	...
	if len(positional) != 1 {
		fmt.Fprintln(stderr, "gti-lookup: cache takes one subcommand: status or clear")
		return exitError
	}
	...
	case "status":
		...
		if jsonOut {
			payload := struct {
				cache.Stats
				ThreatTTLHours float64 `json:"threat_ttl_hours"`
				IOCTTLHours    float64 `json:"ioc_ttl_hours"`
			}{Stats: st, ThreatTTLHours: cfg.ThreatTTL.Hours(), IOCTTLHours: cfg.IOCTTL.Hours()}
		...
		fmt.Fprintf(stdout, "  threat ttl: %g hours\n", cfg.ThreatTTL.Hours())
		fmt.Fprintf(stdout, "  ioc ttl:    %g hours\n", cfg.IOCTTL.Hours())
		if st.Entries > 0 {
			...
			fmt.Fprintln(stdout, "  note: TTLs are applied at read time, so lowering one in the")
	...
	case "clear":
		n, err := store.Clear()
		if err != nil {
			fmt.Fprintf(stderr, "gti-lookup: clear cache: %v\n", err)
			return exitError
		}
		if jsonOut {
			enc := json.NewEncoder(stdout)
	...
	default:
		fmt.Fprintf(stderr, "gti-lookup: unknown cache subcommand %q (want status or clear)\n", positional[0])
		return exitError
	}
}
